...
export default function Sidebar({ activeTab, setActiveTab, isMobile }) {
  return (
    <aside className={`${isMobile ? "w-full h-full" : "hidden md:block w-72"} bg-[#0c061a] text-white border-r border-white/5 transition-colors duration-300`}>
      {/* Brand logo section */}
      <div className="flex h-20 items-center justify-start gap-2 border-b border-white/5 px-6">
        ...
        <span className="font-extrabold tracking-widest text-transparent bg-clip-text bg-gradient-to-r from-white to-slate-400">
          PATHFINDER
        </span>
      </div>
      ...
      <nav className="space-y-2 p-4">
        ...
        >
          <TrendingUp className={`h-5 w-5 md:h-6 md:w-6 ${activeTab === "overview" ? "text-[#FF6C03]" : "text-slate-400"}`} />
        ...
        >
          <BookOpen className={`h-5 w-5 md:h-6 md:w-6 ${activeTab === "courses" ? "text-[#25CCF4]" : "text-slate-400"}`} />
        ...
        >
          <Calendar className={`h-5 w-5 md:h-6 md:w-6 ${activeTab === "schedule" ? "text-[#7545F0]" : "text-slate-400"}`} />
